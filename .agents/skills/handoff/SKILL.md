...
2. **Pull state from these sources (in order):**
   - Session state in `journal.md`; task state in `[bucket]/workstreams/<slug>.md` (if active task docs were in flight).
   ...
   - Session captures written to `threads/` this session (if any).
...
- Session log: `journal.md` · Task docs: `[bucket]/workstreams/<slug>.md` (if a task doc drove the session)
- Session captures: `threads/<slug>.md` (if any were written)
