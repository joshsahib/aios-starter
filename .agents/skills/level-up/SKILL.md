...
Walks the user through the Three Ms each week to surface and ship one new automation. **One interview = one artifact.** It also installs the Three Ms thinking pattern over time — after 4–6 runs, the user starts spotting opportunities mid-week without prompting because the questions have become internal defaults.
...
Read from the **current working directory** first; fall back to root if the file isn't found there.
...
If working inside a bucket folder (`day-job/`, `business-hobby/`, `personal-family/`), prioritize that bucket's context. The level-up session should surface leverage within the active domain, not globally.
...
5. *"What one thing would unlock the most momentum in this domain?"* (growth lever)
...
**Step 5 — Tie to a metric.** Which of the three value levers does this move?
- More throughput / reach
- More quality / value per output
- Less time / cost / friction
...
- **Generic test:** Run with no prompt. Expected: skill asks about the user's week and surfaces 2–3 candidates drawn from their priorities and top_pain — not generic suggestions like "you should build a brief."
...
- **Bucket-aware test.** Run from inside `day-job/`. Expected: context reads from `day-job/context/priorities.md` first.

---

*The Three Ms framework referenced in this skill was developed by Nate Herk. Full framework in `references/3ms-framework.md`.*
