...
Runs the **Four Cs Audit** on the current Claude Code project. Reads (never writes) the project's operating manual, persona, skills, agents, MCPs, decisions, and references. Scores each of the Four Cs out of 25. Surfaces strengths and the top 3 leverage-weighted gaps with concrete next-step commands.
...
| Layer | Test |
|---|---|
| **Context** | Knows the person — identity, domains, voice, decisions, references |
| **Connections** | Reaches their stuff — MCPs, integrations, data sources |
...
|---|---|---|
| Operating manual exists and is substantive (>200 words) | 4 | Read CLAUDE.md, count words |
...
| # | Domain | Examples |
|---|---|---|
| 1 | Revenue / Financials | Stripe, PayPal, QuickBooks, Monarch, personal banking |
| 2 | Customer / relationship interactions | HubSpot, Salesforce, Gmail-as-CRM, DMs |
| 3 | Calendar | Google Cal, Outlook, Fantastical, Calendly |
| 4 | Communication | Gmail, Outlook, Slack, Teams, iMessage |
| 5 | Project / task tracking | ClickUp, Asana, Linear, Notion DB, Jira, Todoist |
| 6 | Meeting intelligence | Granola, Otter, Fireflies, Gong, Zoom |
| 7 | Knowledge / files | Notion, Drive, Dropbox, Confluence, SharePoint |

**Tier-2 (bonus):** AI service API keys (Anthropic, OpenAI, OpenRouter), content/publishing, personal health/fitness data.
...
| 1+ user-built skill | 10 | Skill names not in: `onboard`, `audit`, `level-up`, `grill-with-context`, `skill-creator`, `skill-builder`, `decision`, `connect`, `connect-check`, `memory-prune`, `scaffold-skill`, `scaffold-agent`, `draft`, `standup` (canonical AIOS + Anthropic shipped skills) |
| 1+ agent defined | 5 | Count `.agents/agents/*.md` or `.claude/agents/*.md` ≥ 1 |
...
**Impact multipliers:**
- 0 tier-1 domains reachable: **4x** (AIOS is blind to the person's actual work)
- Operating manual missing or thin: **3x** (foundation)
- SOUL.md missing or all placeholders: **3x** (persona not loaded — every session starts cold)
...
- **SOUL.md unfilled?** "Fill in the bracketed sections — especially Communication preferences and Caution triggers. This file loads in every session."
- **No bucket context?** "Run `/onboard` or manually create `[bucket]/context/priorities.md` with your current priorities for that domain."
...
- **Read-only by default.** Never modify CLAUDE.md, SOUL.md, memory, skills, or any project files. Only optional write is the audit report.
...
- **AGENTS.md check is lightweight.** Verify symlinks exist and resolve — don't penalize if they're dangling (bucket CLAUDE.md not yet created is expected and by design).
